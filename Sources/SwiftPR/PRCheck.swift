...
extension PRCheck {
    ...
    static var checkResultsPrefix: String { "<!--__output-start__" }
    static var checkResultsSuffix: String { "__output-end__-->" }
    ...
    public static func main() async throws {
        ...
        do {
            ...
            // Preserve any previous results from other checks run on this PR.
            if let prCheckComment, let resultsStart = prCheckComment.body.range(of: checkResultsPrefix), let resultsEnd = prCheckComment.body.range(of: checkResultsSuffix) {
                let results = prCheckComment.body[prCheckComment.body.index(after: resultsStart.upperBound)..<resultsEnd.lowerBound]
                let decodedResults = try JSONDecoder().decode(PR.Results.self, from: Data(results.utf8))
                // Save the previous results
                pr.results = decodedResults
            }
            ...
            func createOrUpdateSwiftPRComment() async throws {
                // If this check has run before, replace its resulting output
                // Otherwise, append it to the overall results
                if let checkIndex = pr.results.output.firstIndex(where: { $0.checkName == pr.output.checkName }) {
                    pr.results.output[checkIndex] = pr.output
                } else {
                    pr.results.output.append(pr.output)
                }
